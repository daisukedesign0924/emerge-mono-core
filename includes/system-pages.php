...
function emcore_page_system_pages() {
	...
	foreach ( $types as $key => $meta ) { $s = $sets[ $key ]; $is_open = $created_key ? $created_key === $key : 'contact' === $key; echo '<details class="emcore-panel emcore-system-settings" style="margin-bottom:16px"' . ( $is_open ? ' open' : '' ) . '><summary><strong>' . esc_html( $meta['label'] ) . '</strong><span class="emcore-help">' . ( $s['enabled'] ? '有効' : '無効' ) . '</span></summary><div class="emcore-system-settings-body">';
		...
		if ( 'contact' === $key ) { echo '<div class="emcore-system-form-fields"><span class="emcore-eyebrow">FORM FIELDS</span><h3>フォームフィールド</h3><p>お問い合わせで使用する入力項目です。追加・削除・並べ替えができます。</p><div class="emcore-system-fields" data-form-type="contact">'; foreach ( emcore_system_form_fields( 'contact' ) as $index => $field ) { echo emcore_system_field_admin_row( 'contact', $index, $field ); } echo '</div><button type="button" class="emcore-btn emcore-system-field-add" data-form-type="contact">＋ フィールドを追加</button></div>'; }
